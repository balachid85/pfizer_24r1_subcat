...
class MHCM1(BaseSDQApi):
    ...
    def execute(self):
        study= self.study_id
        sub_cat = self.__class__.__name__ #'MHCM1'
            ...
            try:
                ...
                for ind in range(mh_df.shape[0]):
                    try:
                        ...
                        mhstdat = mh_record[dt_cols[0]].values[0]
                        cm_records = pd.DataFrame()
                        #print('Subject, mhstdat', subject, mhstdat)
                        ...
                        for cm_ind in range(cm_records.shape[0]):
                            ...
                            cmstdat = cm_record[dt_cols[2]].values[0]
                            
                            if utils.compare_partial_date(mhstdat, cmstdat,'>'):
                                subcate_report_dict = {}
                                report_dict = {}
...
if __name__ == "__main__":
    ...
    # python MHCM1.py 1 1 77 1 0.1 > mhcm1log.txt
    study_id = sys.argv[1]
    account_id = sys.argv[2]
    job_id = sys.argv[3]
    rule_id = sys.argv[4]
    version = sys.argv[5]
    rule = MHCM1(study_id, account_id, job_id, rule_id, version)
    rule.run()
