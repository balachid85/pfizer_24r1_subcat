...
class MHCM2(BaseSDQApi):
    domain_list = ['MH','CM']

    def execute(self):
        study= self.study_id
        sub_cat = self.__class__.__name__#'MHCM2'
        ...
        for subject in tqdm.tqdm(subjects):
            ...
            try:
                ...
                try:
                    ...
                    cm_df = cm_df[cm_df[dt_cols[2]].notna()]        
                ...
                for ind in range(mh_df.shape[0]):
                    try:
                        ...
                        try:
                            cm_records = cm_records[cm_records[dt_cols[2]].apply(utils.compare_partial_date, args = (mhendat,'>'))]
                        except Exception as dt_exc:
                            print(f'Exception while comparing dates : {dt_exc}')
                            continue
                        ...
                        for cm_ind in range(cm_records.shape[0]):
                            ...
                            query_text_param={cols['cm_trt_col'][0] : cm_record[cols['cm_trt_col'][0]].values[0], 
                                            dt_cols[1] :  mh_record[dt_cols[1]].values[0],
                                            dt_cols[2] : cm_record[dt_cols[2]].values[0]
                                            } 
                            payload = {
                                "subcategory": sub_cat,
...
if __name__ == "__main__":
    ...
    # python MHCM2.py 1 1 13 1 0.1 > mhcm2log.txt
    study_id = sys.argv[1]
    account_id = sys.argv[2]
    job_id = sys.argv[3]
    rule_id = sys.argv[4]
    version = sys.argv[5]
    rule = MHCM2(study_id, account_id, job_id, rule_id, version)
    rule.run()
