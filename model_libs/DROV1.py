...
#                 ec_df['SUBJECTID'] = ec_df['SUBJID'].astype(float).astype(int)
                ...
                for idx_row in ec_df.index:
                    try:
                        ...
                        if (all([date for date in [ecst_dat, ecend_dat] if date not in ['', ' ', 'null']])):
                            ...
                            new_ec_records = ec_records[(ec_records['ECSTDAT'].notna())]
                            new_ec_records = new_ec_records[(new_ec_records['ECENDAT'].notna())]         
    ...
    #                                 condition5_df.shape[0], condition6_df.shape[0], condition7_df.shape[0], condition8_df.shape[0])
                        ...
                        elif(ecst_dat not in ['', ' ', 'null'] and ecend_dat in ['', ' ', 'null']):
                    ...
                    if (new_ec_record.shape[0] >= 1):
                        ...
                        for ind in range(new_ec_record.shape[0]):
                            ...
                            is_ov_duplicate = check_if_duplicate(subjid= int(cur_ec['SUBJID']), subcat= 'DROV1', index= int(cur_ec[index_idfier]), report= duplicate_report)
