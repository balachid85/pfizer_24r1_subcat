...
class DRAE8(BaseSDQApi):
    ...
    def execute(self):
        study = self.study_id
        sub_cat = 'DRAE8'
        subcat_aeacn_list = ['notequals', 'DOSE REDUCED']
        # subjects = ['16875003']
        subjects = self.get_subjects(study, domain_list = self.domain_list)# [12121240]#
                    ...
                    if (type(ext_result) == tuple) & (type(ext_result) != bool):
                        ...
                        for k, v in ext_result[0].items():
                            for cur_ae in v:
                                ...
                                # print('flag_details',ecadj_flag,aeacn_flag)
                                if (ecadj_flag) and (aeacn_flag):
                                    ...
                                    dose_flag, dose_info = utils.get_dose_values_v1(ec_records, formindex, ecst_dat, ecend_dat)
                                    # print('dose_info',dose_flag, dose_info)
                                    ...
                                    if (dose_flag):
                                        ...
                                        *dose_values, dose_col, prev_itemsetix, prev_visitnm = dose_info
                                        ...
                                        print(ecst_dat,aest_dat,aend_dat)
                                        if (current_dose_value < previous_dose_value) and (current_dose_value != 0):
                                            if (type(aend_dat) != float) and ((ecst_dat >= aest_dat) and (ecst_dat <= aend_dat)):
                                                #print(True, '*'*25)
...
#                                                 debug((ecst_dat >= aest_dat), (ecst_dat <= aend_dat))
                                                ...
                                                ec_itemsetix = cur_ec['ITEMSET_IX'].values[0]
                                                ec_visitnm = cur_ec['VISIT_NM'].values[0]
                                                ae_aespid = cur_ae['AESPID'].values[0]
                                                ae_aeterm = cur_ae['AETERM'].values[0]
                                                param_dict = {'EC_ITEMSETIDX': prev_itemsetix, 
                                                                'AESPID': ae_aespid, 
                                                                'AETERM': ae_aeterm,
                                                                'VISITNAME': prev_visitnm}
...
if __name__ == '__main__':
    ...
    rule_id = sys.argv[4]
    version = sys.argv[5]
    rule = DRAE8(study_id, account_id, job_id, rule_id, version)
    rule.run()
