...
# String Functions:
def nvl(string, replace_with):
    '''Fully support Oracle NVL'''
    if string is None:
        return replace_with

def nvl2(string, value_if_not_null, value_if_null):
...
def substr(string, start_position, leng = 65535):
...
def instr(srcstr, sub_str, start = 0, nth = 1):
...
def to_char(uinput, uformat = None):
...
def trunc(uinput, argv1 = 0):
    '''TRUNC changed from Oracle'''
    if isinstance(uinput, Dateclass):
        return trunc_date(uinput, argv1)
    else:
        return trunc_number(uinput, argv1)
...
def to_date(date_string, uformat):
...
def to_char_date(value, *uformat):
