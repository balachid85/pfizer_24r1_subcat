...
class MHOV1(BaseSDQApi):
    domain_list = ['MH']

    def execute(self):
        study= self.study_id
        sub_cat = self.__class__.__name__ #'MHOV1'    
        f_d = 'display_fields'
        f_c = 'fn_config'
            ...
            try:
                ...
                for ind in range(mh_df.shape[0]):                    
                    try:
                        ...
                        if(hash1 in dup_list):
                            continue
                        stdat = mh_record[dt_cols[0]].values[0]
                        endat = mh_record[dt_cols[1]].values[0]
                            
                        mhterm_pt = str(mh_record[cols['mh_decod_col'][0]].values[0])
                        ...
                        mhongo = mh_record[cols['mh_ongo_col'][0]].values[0] 
                        if pd.isna(mhongo) or mhongo == None or str(mhongo).upper() in vals['mh_ongo_val']['no_val']:
                            temp = mh_df[(mh_df[cols['uniq_id_col'][0]] != hash1) & \
                                (mh_df[dt_cols[0]].apply(utils.compare_partial_date,args = (stdat,'==',))) & \
                                (mh_df[dt_cols[1]].apply(utils.compare_partial_date,args = (endat,'==',)))] 
                                #(mh_df[cols['mh_ongo_col'][0]] == str(mhongo).upper())]
                            
                                # & (mh_df[cols['mh_tox_col'][0]].str.upper() == mhtoxgr.upper())]
                        elif (not pd.isna(mhongo)) and mhongo != None and str(mhongo).upper() in vals['mh_ongo_val']['yes_val']:
                            temp = mh_df[(mh_df[cols['uniq_id_col'][0]] != hash1) & \
                                (mh_df[dt_cols[0]].apply(utils.compare_partial_date,args= (stdat,'==',))) & \
                                (mh_df[cols['mh_ongo_col'][0]] == str(mhongo).upper())]
                                # (mh_df[cols['mh_tox_col'][0]].str.upper() == mhtoxgr.upper())]                        
                        ...
                        if mhtoxgr_present:
                            mhtoxgr = str(mh_record[cols['mh_tox_col'][0]].values[0])  
                            temp[cols['mh_tox_col'][0]] = temp[cols['mh_tox_col'][0]].apply(lambda x: str(x).upper())
                            temp = temp[temp[cols['mh_tox_col'][0]].str.upper() == str(mhtoxgr).upper()]
                        ...
                        if len(new_mh_record) > 0:
                            ...
                            for new_ind in new_mh_record.index.tolist():
                                curr_mh_rec = new_mh_record.loc[[new_ind]]
                                curr_mhdecod = str(curr_mh_rec[cols['mh_decod_col'][0]].values[0])                                   
                                mhdecod_flag = utils.check_similar_medication_verabtim2(mhterm_pt.upper(), curr_mhdecod.upper())
                                if mhdecod_flag:
                                    final_mh_df = final_mh_df.append(curr_mh_rec, ignore_index = True)
                                    total_valid_pred += 1
                        ...
                        #print('Report dict is :', report_dict)
                        subcate_report_dict[sub_cat] = report_dict
                        query_text_param={cols['mh_decod_col'][0] : mh_record[cols['mh_decod_col'][0]].values[0],
                                            dt_cols[0]: mh_record[dt_cols[0]].values[0],
                        ...
                        print('Exception is :', exc1)
                        print(traceback.format_exc())
                        continue
            except Exception as exc2:
                print('Exception is :', exc2)
                print(traceback.format_exc())
                continue
            if self.has_auto_closure(self.study_id,  self.rule_id, self.version):
                self.close_query(subject, payload_list)
if __name__ == "__main__":
    ...
    #python MHOV1.py 1 1 1 82 0.1 > mhov1log.txt
    study_id = sys.argv[1]
    account_id = sys.argv[2]
    job_id = sys.argv[3]
    rule_id = sys.argv[4]
    version = sys.argv[5]
    rule = MHOV1(study_id, account_id, job_id, rule_id, version)
