...
import traceback, tqdm, logging, numpy as np, os, yaml
                                        ...
                                        if aecmgiv.upper() == 'YES':
                                            ...
                                            if drug_flag:
                                                ...
                                                if valid_drug == False:
                                                    ...
                                                    ae_record = ae_record.iloc[0]
                                                    payload = {'subcategory':sub_cat,
                                                     'cdr_skey':str(ae_record1['cdr_skey']), 
                                                     'query_text':self.get_model_query_text_json(study, sub_cat, params=[formidx, cmtrt]), 
                                                     'form_index':str(ae_record['form_index']), 
                                                     'question_present':self.get_subcategory_json(study, sub_cat), 
                                                     'modif_dts':str(ae_record['modif_dts']), 
...
if __name__ == '__main__':
    ...
    rule_id = sys.argv[2]
    version = sys.argv[3]
    rule = AECM7(study_id, rule_id, version)
    rule.run()