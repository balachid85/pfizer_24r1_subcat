            ...
            try:
                ...
                for ind in range(ae_df.shape[0]):
                    try:
                        ...
                        o_aespid = ae_record[cols['aespid_col'][0]].values[0]
                            ...
                            if len(ae_cm_records[0]) > 0:      
                                for ae_formindex, cm_records in ae_cm_records[0].items():
                                        for cm_record in cm_records:
                                            new_ae_records, new_cm_record = utils.get_cm_hierarchy(ae_record, ae_df, cm_record, key = cols['aeendat_dts_col'][0])
                                            ...
                                            for aeptcd in new_ae_records[cols['aeptcd_col'][0]].unique().tolist():
                                                ...
                                                new_ae_record[cols['aeongo_col'][0]] = new_ae_record[cols['aeongo_col'][0]].astype(str)
                                                aeongos = new_ae_record[cols['aeongo_col'][0]].unique().tolist()
                                                ...
                                                if type(new_ae_record) != int:
                                                    ...
                                                    dtr_null_flag=False
                                                    if cols['aeendat_dtr_col'][0] in new_ae_record.columns:
                                                        if new_ae_record[cols['aeendat_dtr_col'][0]].astype('str').values[0] in utils.null_list: 
                                                            dtr_null_flag=True
                                                    
#                                                     ae_dtr_flag = False
                                                    if cols['aeendat_dtr_col'][0] in new_ae_record.columns and not dtr_null_flag:
#                                                         ae_dtr_flag = True
                                                        new_ae_record[cols['aeendat_dtr_col'][0]] =  new_ae_record[cols['aeendat_dtr_col'][0]].astype('str')
                                                        ae_endt = new_ae_record[cols['aeendat_dtr_col'][0]].values[0]
                                                    else:
                                                        new_ae_record[cols['aeendat_dts_col'][0]] = new_ae_record[cols['aeendat_dts_col'][0]].astype('str')
                                                        ae_endt = new_ae_record[cols['aeendat_dts_col'][0]].values[0]
