...
# Embedded file name: /users/sdqdev/gowtham/pfizer_data_upload/dags/scripts/SubcatModels/model_libs/AEDR10.py
# Compiled at: 2021-10-20 15:52:00
...
import traceback, tqdm, numpy as np, logging, yaml, numpy as np, os
                    ...
                    try:
                        ...
                        for i, (drug, records) in enumerate(ec_dict.items()):
                            ...
                            if len(new_ec) > 1:
                                ...
                                washout_date = ecendat + np.timedelta64(30, 'D')
                                if aest_dat.values[0] > washout_date:
                                    payload_flag = True
                            else:
                                if len(new_ec) == 1:
...
if __name__ == '__main__':
    ...
    rule_id = sys.argv[2]
    version = sys.argv[3]
    rule = AEDR10(study_id, rule_id, version)
    rule.run()