...
class MHCM4(BaseSDQApi):
    domain_list = ['MH','CM']    

    def execute(self):
        study= self.study_id
        sub_cat = self.__class__.__name__ #'MHCM4'
        ...
        try:
            ...
            if(match_term.upper() == 'MATCH_DSL'):
                ...
                #match_config = fn_config[match_term] #or
        ...
        for subject in tqdm.tqdm(subjects):
            ...
            try:
                ...
                try:
                    ...
                    if(len(mh_df) == 0 or len(cm_df) == 0):
                        continue

                    mh_df = mh_df[mh_df[dt_cols[1]].notna()]
                    cm_df = cm_df[cm_df[dt_cols[3]].notna()]
                    ...
                    try:
                        ...
                        if(not match_dsl_flag):
                            try:
                                ...
                                continue
                            #print('match flag : ',match_flag, cm_records.shape)                    
                        else:
                            try:
                                if smc:
                                    predict_med_code = self.predict_medical_code
                        ...
                        for cm_ind in range(cm_records.shape[0]):
                            ...
                            cmendat = cm_record[dt_cols[3]].values[0]
                           
                            if utils.compare_partial_date(cmendat, mhendat, '>') :
                                #print(mhendat, cmendat)
                                subcate_report_dict = {}
                                report_dict = {}
                                ...
                                query_text_param={dt_cols[1] : mh_record[dt_cols[1]].values[0],
                                                  dt_cols[3] : cm_record[dt_cols[3]].values[0] 
                                                } 
...
if __name__ == "__main__":
   #(self.study_id, self.account_id, self.job_id, rule['ml_model_id'], 0.1
    # python MHCM4.py 1 1 79 1 0.1 > mhcm4log.txt
    study_id = sys.argv[1]
    account_id = sys.argv[2]
    job_id = sys.argv[3]
    rule_id = sys.argv[4]
    version = sys.argv[5]
    rule = MHCM4(study_id, account_id, job_id, rule_id, version)
    rule.run()
