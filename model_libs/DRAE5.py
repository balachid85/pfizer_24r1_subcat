...
# Embedded file name: DRAE5.py
# Compiled at: 2021-05-11 13:45:21
# Size of source mod 2**32: 10437 bytes
import re, pdb, sys, yaml, logging, warnings, traceback, numpy as np, pandas as pd
try:
...
class DRAE5(BaseSDQApi):
    ...
    def execute(self):
        ...
        study = self.study_id
        sub_cat = 'DRAE5'
        subcat_aeacn_list = ['DOSE REDUCED']
                    ...
                    if (type(ext_result) == tuple) & (type(ext_result) != bool):
                        ...
                        for k, v in ext_result[0].items():
                            for cur_ae in v:
                                ...
                                print('ecadj_flag',ecadj_flag,aeacn_flag,aeacn_tuple)
                                if ecadj_flag:
                                    if aeacn_flag:
                                        ecrecord_flag, ec_values = utils.get_ecrecords(cur_ec, cur_ae, ec_df, ae_df)
                                        print('ecrecord_flag',ec_values)
                                    ...
                                    print('dose_flag',dose_info,dose_flag)
                                    if dose_flag:
                                        *dose_values, dose_col, prev_itemsetix, prev_visitnm = dose_info
                                        ...
                                        query_flag = False                                        
                                        if previous_dose_value > current_dose_value:
                                            ...
                                            if query_flag:
                                                ...
                                                param_dict = {'EC_ITEMSETIDX':cur_ec['ITEMSET_IX'].values[0], 
                                                 ...
                                                 'AETERM':cur_ae['AETERM'].values[0],  'AESTDAT':fmt(cur_ae['AESTDAT'].values[0]), 
                                                 'AEENDAT':fmt(cur_ae['AEENDAT'].values[0]),  'ECSTDAT':fmt(cur_ec['ECSTDAT'].values[0]), 
                                                 'ECENDAT':fmt(cur_ec['ECENDAT'].values[0])}
...
if __name__ == '__main__':
    ...
    rule_id = sys.argv[4]
    version = sys.argv[5]
    rule = DRAE5(study_id, account_id, job_id, rule_id, version)
    rule.run()