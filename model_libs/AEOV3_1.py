...
class AEOV3_1(BaseSDQApi):
    ...
    def execute(self):
        ...
        subjects = self.get_subjects(study, domain_list=(self.domain_list), per_page=10000) #[88880014]#
            ...
            try:
                ...
                for ind in range(ae_df.shape[0]):
                    try:
                        ...
                        ae_records = ae_df
                        ae_records = ae_records[ae_records['AESEV'].notna()]
                        new_ae_record = pd.DataFrame()
                        if type(aest_dt.values.tolist()[0]) != float:
                            if type(aeen_dt.values.tolist()[0]) != float:
                                if aeongo.upper() == 'NO':
                                    ...
                                    if aesev_present:
                                        ae_record1 = ae_records[((ae_records['AESTDAT'] >= aest_dt.values[0]) & (ae_records['AESTDAT'] < aeen_dt.values[0]) & (ae_records['AESEV'] != aesev))]
                                        ae_record2 = ae_records[((ae_records['AESTDAT'] > aest_dt.values[0]) & (ae_records['AESTDAT'] < aeen_dt.values[0]) & (ae_records['AESEV'] != aesev))]
                                    ...
                                    ae_record['AEENDAT'] = ae_record['AEENDAT'].dt.strftime('%d-%b-%Y')
                        if type(aest_dt.values.tolist()[0]) != float:
                            if aeongo.upper() == 'YES':
                                ae_records = ae_records[ae_records['AESTDAT'].notna()]
                                ae_records['AESTDAT'] = ae_records['AESTDAT'].apply(utils.get_date)
                                ae_records['AEENDAT'] = ae_records['AEENDAT'].apply(utils.get_date)
                                if aesev_present:
                                    ae_record1 = ae_records[((ae_records['AESTDAT'] > aest_dt.values[0]) & (ae_records['AESEV'] != aesev))]
                                    ae_record2 = ae_records[((ae_records['AESTDAT'] >= aest_dt.values[0]) & (ae_records['AEONGO'] != aeongo) & (ae_records['AESEV'] != aesev))]
                                    new_ae_record = new_ae_record.append(ae_record1, ignore_index=True)
                                    new_ae_record = new_ae_record.append(ae_record2, ignore_index=True)
                                else:
                                    ...
                                    if aeterm_flag:
                                        if aedecod_flag:
                                            ...
                                            if curr_aeongo.upper() == 'NO':
                                                new_ae_record1['AEENDAT'] = fmt(new_ae_record1['AEENDAT'].values[0])
                                            piv_rec = {'AE1':ae_record.head(1), 
                                             'AE2':new_ae_record1.head(1)}
                                        ...
                                        query_text_param = {'AESPID':aespid, 
                                         ...
                                         'AEONGO1':aeongo1, 
                                         'AESEV':aesev}
                                        is_ov_duplicate = check_if_duplicate( subjid=(int(ae_record1['subjid'])), subcat='AEOV3_1', index=(int(ae_record1['form_index'])), report=subcate_report_dict)
                                        print('check_if_duplicate',check_if_duplicate,subject)
...
if __name__ == '__main__':
    ...
    rule_id = sys.argv[4]
    version = sys.argv[5]
    rule = AEOV3_1(study_id, account_id, job_id, rule_id, version)
    rule.run()