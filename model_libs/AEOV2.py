...
class AEOV2(BaseSDQApi):
    ...
    def execute(self):
        ...
        subjects = self.get_subjects(study, domain_list = self.domain_list, per_page = 10000)#[10021005]#
...
#                                                 rep_df=rep_df.T.drop_duplicates().T
                                    ...
                                    is_ov_duplicate = check_if_duplicate(subjid= int(ae_record1['subjid']), subcat='AEOV2', index=int(ae_record1['form_index']), report=subcate_report_dict)
                                    #not handled in 23R1
                                    # is_ov_duplicate = False
                                    print("is ov duplicate",is_ov_duplicate)
...
if __name__ == '__main__':
    ...
    rule_id = sys.argv[4]
    version = sys.argv[5]
    rule = AEOV2(study_id, account_id, job_id, rule_id, version)
    rule.run()
