...
# Embedded file name: /users/sdqdev/dhilip/subcats_saama/pfizer_data_upload_py/pfizer_data_upload/dags/scripts/SubcatModels/model_libs/AECM2_1.py
...
Changes: 
16-08-22 -  [EDIT #2] extracting only the date Form the impute date
                                        ...
                                        for cm_record in cm_records:
                                            ...
                                            for aeptcd in new_ae_records['AEPTCD'].unique().tolist():
                                                ...
                                                if type(new_ae_record) != int:
                                                    ...
                                                    unk_flag, year_not_unk, date_dict = utils.impute_unk({'aeendat':ae_endt,  'cmstdat':cm_stdt})
                                                    if not year_not_unk:
                                                        continue
...
if __name__ == '__main__':
    ...
    rule_id = sys.argv[4]
    version = sys.argv[5]
    rule = AECM2_1(study_id, account_id, job_id, rule_id, version)
    rule.run()