...
class DHAE2(BaseSDQApi):
    domain_list = ['AE','DD']

    def execute(self):
        study = self.study_id
        domain_list = self.domain_list
        sub_cat = self.__class__.__name__ #'DHAE2'
        ...
        for subject in tqdm.tqdm(subjects):
            ...
            try:
                    ...
                    if len(dh_df) == 0 or len(ae_df)==0:
                        ...
                        for i in cols['ae_ref_id_col']:
                            if i in ae_cols:
                                aerefid = i
                                break   
                        ae_df[cols['ae_ser_col'][0]] = ae_df[cols['ae_ser_col'][0]].apply(lambda x: str(x).upper())
                        ae_df = ae_df[~(ae_df[cols['ae_ser_col'][0]].isin(vals['ae_ser_val']))]
                ...
                for ind in range(ae_df.shape[0]):
                    ...
                    for dh_ind in range(new_dh_df1.shape[0]):
                        dh_record = new_dh_df1.iloc[[dh_ind]]
                        subcate_report_dict = {}
                        report_dict = {}                 
                        ...
                        query_text_param={
                                cols['dh_ae_id_col'][0] : dh_record[cols['dh_ae_id_col'][0]],
                                aerefid : ae_record[aerefid].values[0],
                                cols['ae_ser_col'][0] : ae_record[cols['ae_ser_col'][0]].values[0]
                            }
...
if __name__ == '__main__':
    ...
    # python DHAE2.py 1 1 52 1 0.1 > dhae2log.txt
    study_id = sys.argv[1]
    account_id = sys.argv[2]    
    rule_id = sys.argv[3]
    job_id = sys.argv[4]
    version = sys.argv[5]
    rule = DHAE2(study_id, account_id, job_id, rule_id, version)
