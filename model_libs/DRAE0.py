...
# Embedded file name: DRAE0.py
# Compiled at: 2021-02-25 19:08:53
# Size of source mod 2**32: 8858 bytes
import re, sys, yaml, logging, warnings, traceback, numpy as np, pandas as pd
try:
                ...
                for i_row in range(ec_df.shape[0]):
                    ...
                    if (type(ext_result) == tuple) & (type(ext_result) != bool):
                        ...
                        for k, v in ext_result[0].items():
                            for cur_ae in v:
                                ecadj_flag, ecadjo_col, ecadjo_val = utils.check_ecadj(cur_ec, rule_flag='other')
                                aeterm = cur_ae['AETERM'].iloc[0]
                                if ecadj_flag:
                                    ...
                                    if ecst_dat >= aest_dat and utils.check_similar_aeterm(aeterm, ecadjo_val):
                                        print(True, '*' * 25)
                                        subcat_report = dict()
                                        report_dict = dict()
                                        ...
                                        param_dict = {'EC_ITEMSETIDX':ec_itemsetix, 
                                         'ECADJO':ecadjo_val,  'AETERM':aeterm, 
                                         'AESPID':ae_aespid}
...
if __name__ == '__main__':
    ...
    rule_id = sys.argv[2]
    version = sys.argv[3]
    rule = DRAE0(study_id, rule_id, version)
    rule.run()