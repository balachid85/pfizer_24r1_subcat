...
class MHOV2(BaseSDQApi):
    domain_list = ['MH']

    def execute(self):
        study= self.study_id
        sub_cat = self.__class__.__name__ #'MHOV2'
        f_d = 'display_fields'
        f_c = 'fn_config'
        ...
        for subject in tqdm.tqdm(subjects):
            ...
            try:
                ...
                try:
                    mh_df = pd.DataFrame(flatten_data.get('MH',[]))
                    if(len(mh_df)>0):
                ...
                for ind in range(mh_df.shape[0]):
                    try:
                        ...
                        mhst_dt = mh_record[dt_cols[0]].values[0]
                        mhend_dt = mh_record[dt_cols[1]].values[0]
                        ...
                        mh_records = mh_records[~((mh_records[dt_cols[0]].apply(utils.compare_partial_date, args =(mhst_dt,'=='))) & (mh_records[dt_cols[1]].apply(utils.compare_partial_date, args =(mhend_dt,'=='))))]
                        ...
                        #if type(mhst_dt) != float and type(mhend_dt) != float: 
                            new_mh_record = mh_records[(mh_records[dt_cols[0]].apply(utils.compare_partial_date, args =(mhst_dt,'>='))) & \
                                            (mh_records[dt_cols[0]].apply(utils.compare_partial_date, args =(mhend_dt,'<')))]    
                        else:
                            mh_record1 = mh_records[(mh_records[dt_cols[0]].apply(utils.compare_partial_date, args =(mhst_dt,'>')))] # & \
                            mh_record2 = mh_records[(mh_records[dt_cols[0]].apply(utils.compare_partial_date, args =(mhst_dt,'==')))  & \
                        ...
                        if len(new_mh_record) >= 1:
                            ...
                            for new_ind in new_mh_record.index.tolist():
                                ...
                                mhdecod_flag = utils.check_similar_medication_verabtim2(mhdecod.upper(), str(curr_mhdecod).upper())
...
if __name__ == "__main__":
    ...
    #python MHOV2.py 1 1 1 83 0.1 > mhov2log.txt
    study_id = sys.argv[1]
    account_id = sys.argv[2]
    job_id = sys.argv[3]
    rule_id = sys.argv[4]
    version = sys.argv[5]
    rule = MHOV2(study_id, account_id, job_id, rule_id, version)
