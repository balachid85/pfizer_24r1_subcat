...
class DRAE9(BaseSDQApi):
    ...
    def execute(self):
        study = self.study_id
        sub_cat = 'DRAE9'
        subcat_aeacn_list = ['notequals', 'DRUG INTERRUPTED']
        subjects =self.get_subjects(study, domain_list = self.domain_list) ##[12121243,12121242]#['10252002']#
...
#                 ae_df['SUBJECTID'] = ae_df['SUBJID'].astype(float).astype(int)
                ...
                for i_row in range(ec_df.shape[0]):
                    ...
                    subj = cur_ec['SUBJECTID'].iloc[0]
                    ectrt = cur_ec['ECTRT'].iloc[0]
                    ...
                    if (type(ext_result) == tuple) & (type(ext_result) != bool):
                        ...
                        for k, v in ext_result[0].items():
                            for cur_ae in v:
                                ...
                                # print('ecadj_flag',ecadj_flag,aeacn_flag)
                                if (ecadj_flag) and (aeacn_flag):
                                    ...
                                    # print('dose_flag',dose_info)
                                    if (dose_flag):
                                        dose_values, dose_col = dose_info
                                        current_dose_value= int(dose_values)
                                        #import pdb;pdb.set_trace()
                                        if (current_dose_value == 0):
...
#                                                 debug((ecst_dat >= aest_dat), (ecst_dat <= aend_dat))
                                                ...
                                                param_dict = {'EC_ITEMSETIDX': cur_ec['ITEMSET_IX'].values[0], \
                                                              ...
                                                              'AESPID': cur_ae['AESPID'].values[0], 'AETERM': cur_ae['AETERM'].values[0],'VISITNAME': \
                                                              cur_ae['VISIT_NM'].values[0]}
...
if __name__ == '__main__':
    ...
    rule_id = sys.argv[4]
    version = sys.argv[5]
    rule = DRAE9(study_id, account_id, job_id, rule_id, version)
    rule.run()
